...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "OWO.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "OWOd.pid"));
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                if (!fMatch)
                {
                    ...
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("OWO"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
